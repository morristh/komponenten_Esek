\begin{center}
    \vspace*{1.5cm}
    {\fontsize{20}{20}\textbf{\fontspec{Lucida Sans Unicode}Slaskvisor}}\\
    \vspace{0.7cm}
...
\begin{textblock*}{3cm}(7.6 cm, 3.2cm) % {width}(x, y)
    %\scalebox{-1}[1]{\includegraphics[width=2.4cm]{./bilder/majas-bilder/bamse-1.png}}
...
\index[anfa]{Jag skall festa, ta det lugnt med spriten}
\songinfo{Mel: Bamse\\
Sångarstriden 1987}
...
\vissteduatt{Visste du att denna sång tillsammans med Vikingen var de\\första sångerna som lades in i denna upplaga av sångboken?}

\newpage
\resetBackground

\subsection*{Ångestvisan} 
\index[alfa]{xngstvisan@Ångestvisan}
\index[anfa]{Jag vaknar upp på söndan}
...
\index[anfa]{Vi som oss för att glupa satt, supa glatt}
...
\index[anfa]{Mat och vin och öl och sprit serveras här}
...
\index[alfa]{D:arnas fel}
\index[anfa]{D:arnas fel}
\songinfo{Mel: Bögarnas fel\\
Text: Liam Liljeqvist E21}

\begin{parse lines}[\noindent]{#1\\}
    Över hela LTH (Oh Yeah, Oh Yeah)
    Finns inte någon hjälp att få (Å ack o ve)
    Men vi som gå på E-sek, har ändå ryggen fri
    För roten till E-husets ondska, det är inte vi

    Nej, det är D:arnas fel (D:arnas fel)
    D:arnas fel (D:arnas fel)
    I HeHE står det skrivet det är D:arnas fel
    Jag vet inte om i stadgarna, reglementet eller mer,
    men nånstans där i står det, det är D:arnas fel

    Att rosa ofta stavas fel (D:arnas fel)
    Att socialt så är jag stel (D:arnas fel)
    Att ouvvarna är sena och allt annat hemskt
    Att vin och sprit och öl och punschen inte ens är svensk?

    Ja, det är D:arnas fel (D:arnas fel)
    D:arnas fel (D:arnas fel)
    Skolan står i brand och det är D:arnas fel
    Igår jag gick på sittning där tre människor söp ihjäl
    På nått sätt kan jag känna det var D:arnas fel

    Deadlinesen och slut på sprit? (D:arnas fel)
    Att tentan gick så jävla skit? (D:arnas fel)
    Att nollor nu för tiden aldrig lär sig veta hut
    Att de gamla och de dryga numer aldrig tjatat slut
\end{parse lines}

\vissteduatt{Visste du att E-sektionen hade en egen sångbok, innan kåren \\bildades, som sedan slutade användas?}
...
\begin{parse lines}[\noindent]{#1\\}
    Ja, det är D:arnas fel (D:arnas fel)
    D:arnas fel (D:arnas fel)
    Allt är upp och ner och det är D:arnas fel
    Sprit har gjort att jag nog aldrig kan bli psykiskt hel,
    men min sektion är enig det är D:arnas fel

    Att campus är en enda skräck (D:arnas fel)
    Kuggat tre tentor i sträck (D:arnas fel)
    Och på studenters grunder, vi sjunger, super loss
    Men skulden till att det är så, den faller ej på oss

    Nej, det är D:arnas fel (D:arnas fel)
    D:arnas fel (D:arnas fel)
    Därom kan vi enas, det är D:arnas fel
    Man tror att det är tentan själv som spelar oss ett spel
    men även den går fri för allt är, D:arnas fel

    \textit{(Sjunges långsamt)}
    Ja, det är D:arnas fel (D:arnas fel)
    D:arnas fel (D:arnas fel)
    Allting som är fel, ja det är D:arnas fel
    Jag har suttit ner med proggen så min nacke blivit stel
    Det råder inga tvivel, det är D:arnas fel
\end{parse lines}
\newpage
...
\index[anfa]{Man kan dricka vatten, mjölk och gammalt flott}
...
\index[anfa]{Jag hulkar mig i spyorna}
...
\songinfo{Mel: Du käre lille snickerbo\\
...
\index[anfa]{När temperaturen är hög uti kroppen}
\songinfo{Mora Träsk}
